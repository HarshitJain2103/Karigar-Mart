...
export default function ArtisanStorePage() {
    ...
    useEffect(() => {
        const fetchStoreData = async () => {
            try {
                ...
                if (!response.ok) throw new Error(t('artisanStorePage.errors.notFound'));
    ...
    if (error) return <div className="text-center py-20 text-red-500">{t('common.error')}: {error}</div>;
    if (!storeData) return <div className="text-center py-20">{t('artisanStorePage.errors.storeNotFound')}</div>;

    const { profile, products } = storeData;

    // Format member since date
    const formatMemberSince = (date) => {
        if (!date) return t('artisanStorePage.creator');
        ...
        <div className="min-h-screen bg-slate-50">
            <div className="relative h-64 md:h-80 w-full">
                <img
                    ...
                    alt={t('artisanStorePage.bannerAlt', { store: profile.storeName })}
                ...
                <div className="absolute bottom-0 left-0 w-full p-4 md:p-8 text-white">
                    <div className="max-w-7xl mx-auto flex flex-col sm:flex-row justify-between items-center">
                        ...
                        <div className="flex gap-2 mt-4 sm:mt-0">
                            <Button variant="secondary">{t('artisanCard.follow')}</Button>
            ...
            <div className="max-w-7xl mx-auto py-12 px-4">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                    ...
                    <div id="about" className="lg:col-span-8">
                        <h2 className="text-3xl font-bold mb-4 border-b pb-2">{t('artisanStorePage.aboutStore')}</h2>
                    ...
                    <div className="lg:col-span-4">
                        <Card>
                            <CardHeader>
                                <CardTitle>{t('artisanStorePage.aboutArtisan')}</CardTitle>
                            </CardHeader>
                            <CardContent className="space-y-4">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <div className="text-sm text-muted-foreground">
                                            {profile.craft || t('artisanStorePage.creator')}
                                        </div>
                                    </div>
                                </div>
                                <Separator />
                                <div className="flex items-center gap-3 text-sm text-muted-foreground">
                                    ...
                                    <span>{t('artisanStorePage.basedIn', {
                                ...
                                {profile.userId?.createdAt && (
                                    <>
                                        ...
                                        <div className="flex items-center gap-3 text-sm text-muted-foreground">
                                            ...
                                            <span>{t('artisanStorePage.memberSince', {
                                                date: formatMemberSince(profile.userId.createdAt),
                                            })}</span>
                                        </div>
                                    </>
                                )}
                ...
                {/* Products Section */}
                <div>
                    <h2 className="text-3xl font-bold mb-6 text-center">{t('artisanStorePage.collection')}</h2>

    ...
    return (
      <div className="container mx-auto py-12 px-4">
        <div className="mb-8 text-center">
          <h1 className="text-3xl font-bold tracking-tight">{t('cartPage.title')}</h1>
        </div>
        <div className="text-center py-16 border-2 border-dashed rounded-lg">
          <h2 className="text-xl font-semibold">{t('cartPage.emptyTitle')}</h2>
          <p className="text-muted-foreground mt-2">{t('cartPage.emptySubtitle')}</p>
          <Link to="/shop">
            <Button className="mt-6">{t('cartPage.continueShopping')}</Button>
          </Link>
        </div>
      </div>
    );
  ...
  return (
    <div className="container mx-auto py-12 px-4">
      <div className="mb-6 text-center">
        <h1 className="text-3xl font-bold tracking-tight">{t('cartPage.title')}</h1>
        <p className="text-sm text-muted-foreground mt-1">{totalItems} {totalItems !== 1 ? t('cartPage.itemsCountPlural') : t('cartPage.itemsCount')}</p>
      </div>
          ...
          <div className="flex items-center gap-2">
            ...
            {freeShipRemaining > 0 ? (
              <span className="text-sm">
                {t('cartPage.freeShippingRemaining', { amount: freeShipRemaining.toFixed(0) })}
              </span>
            ) : (
              <span className="text-sm font-semibold">{t('cartPage.freeShippingUnlocked')}</span>
            )}
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        <div className="lg:col-span-2 space-y-4">
                ...
                <div className="ml-4 flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      {!isQuantityAvailable && isItemInStock && (
                        <p className="text-sm text-destructive mt-1 font-medium">{t('cartPage.onlyLeft', { count: product.stockQuantity })}</p>
                      )}
                      {!isItemInStock && (
                        <p className="text-sm text-destructive mt-1 font-medium">{t('cartPage.outOfStock')}</p>
                      )}
                    </div>
                    <div className="text-right shrink-0">
                      <div className="font-semibold">₹{Number(product?.price || 0).toFixed(2)}</div>
                      <div className="text-xs text-muted-foreground mt-1">{t('cartPage.each')}</div>
                    </div>
                  </div>
                  ...
                  <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
                    ...
                    <div className="text-right ml-auto sm:ml-0">
                      <div className="text-sm text-muted-foreground">{t('cartPage.itemTotal')}</div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <Link to="/shop">
              <Button variant="outline">{t('cartPage.continueShopping')}</Button>
            </Link>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center gap-2 bg-muted/40 rounded-md p-3">
                ...
                <span className="text-sm">{t('cartPage.secureCheckout')}</span>
              </div>
              ...
              <div className="flex justify-between">
                ...
                <span>{checkoutSubtotal >= freeShipThreshold ? t('cartPage.shippingFree') : t('cartPage.shippingCalculated')}</span>
              </div>
              <Separator />
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium flex items-center gap-2">
                  ...
                  {t('cartPage.orderNote')}
                </label>
                <textarea
                  value={note}
                  onChange={(e) => setNote(e.target.value)}
                  className="w-full rounded-md border bg-background p-2 text-sm"
                  placeholder={t('cartPage.orderNotePlaceholder')}
                  rows={3}
                />
              </div>
            </CardContent>
            <CardFooter className="flex flex-col gap-2">
              ...
              <div className="grid grid-cols-3 gap-2 w-full text-xs text-muted-foreground">
                <div className="flex items-center gap-2 border rounded-md p-2">
                  ...
                  {t('cartPage.fastShipping')}
                </div>
                <div className="flex items-center gap-2 border rounded-md p-2">
                  <Shield className="h-4 w-4 text-primary" />
                  {t('cartPage.buyerProtection')}
                </div>
                <div className="flex items-center gap-2 border rounded-md p-2">
                  <RefreshCw className="h-4 w-4 text-primary" />
                  {t('cartPage.easyReturns')}
                </div>
              </div>
            </CardFooter>
          </Card>
